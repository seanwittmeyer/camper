...
/* 
 * Camper Leader / Events / All View
 *
 * This is the listing of all events in the system that the unit is eligible to register for.
 * It is not a list of events that the unit has registered for. 
 *
 * File: /application/views/events/all.php
...
?>
	<div class="subnav">
		<div class="container">
			...
			<nav class="campersubnav">
				<li class="active"><?php echo anchor("events/all", 'All Events');?></li>
			</nav>
		</div>
	</div>
	<article class="textsection">
		<div class="container">
			...
			<?php if ($individual) { ?>
			<p>Upcoming events are all of the events you can register for. This includes summer camps, council and district events like weekends and camporees, and even trainings. If you wish to register for an event, simply click the register button.</p>
			<p><strong>Already registered?</strong> Head over to the Registrations section to see all events you have registered for. <?php echo anchor('registrations', 'My Registrations &rarr;', 'class="btn btn-small tan"'); ?></p>
			<?php } else { ?>
			<p>Upcoming events are all of the events you can register your unit for. This includes summer camps, council and district events like weekends and camporees, and even trainings. If you wish to register for an event, simply click the register button.</p>
			<p><strong>Already registered?</strong> Head over to the Registrations section to see all events <?php echo $unittitle; ?> has registered for. <?php echo anchor('registrations', $unittitle.' Registrations &rarr;', 'class="btn btn-small tan"'); ?></p>
			<?php } ?>
					...
					<div id="upcoming<?php echo $i; ?>" class="accordion-body collapse ">
						<div class="accordion-inner">
							...
							<div class="desc left">
								...
								<?php else : ?><p><?php echo ($sessions[0]['open'] == '1' && $sessions[0]['count'] < $sessions[0]['limitsoft'] || $sessions[0]['limitsoft'] == 0) ? anchor("registrations/new/".$event['id'], 'Register &rarr;', 'class="btn blue"') : '<div class="alert">Registration is closed since this event is full</div>'; ?></p><?php endif; ?>
							</div>
							...
							<?php if ($sessionscount == 1) : ?><p>
									...
									<strong>Openings:</strong> <?php if ($sessions[0]['open'] == '1' && $sessions[0]['count'] < $sessions[0]['limitsoft']) {
									  			...
									  			} elseif ($sessions[0]['limitsoft'] == 0 && $sessions[0]['limithard'] == 0) { ?>There is no limit for this event.<?php 
											...
											<td><?php foreach ($groups['groups'] as $g) { 
												...
												if ($onesession['open'] == '1') {
													...
													if ($g['__limit'] == 0 || $g['__limit'] == false) {
														...
														$g['__openings'] = '';
														$g['__message'] = '';
														$g['__button'] = anchor('registrations/new/'.$event['id'].'/'.$onesession['id'].'/'.$g['id'], 'Register &rarr;', 'class="btn btn-small blue camperhoverpopover" data-toggle="popover" data-placement="top" data-content="There is no registration limit for this '.strtolower($event['sessiontitle']).'<br /><br />Click to register, you can set your numbers on the next page."');
														...
														$g['__message'] = '';
														$g['__button'] = anchor('registrations/new/'.$event['id'].'/'.$onesession['id'].'/'.$g['id'], 'Register &rarr;', 'class="btn btn-small blue camperhoverpopover" data-toggle="popover" data-placement="top" data-content="<strong>'.$g['__openspots'].'</strong> Openings!"');
													} else {
														// Group Full
										...
										<?php $j=1; foreach ($sessions as $onesession): $sessioncount = $this->shared->count_session($onesession['id']); ?> 
							      	    <tr>
								  			...
								  			<td><?php if ($onesession['open'] == '1' && $sessioncount['total'] < $onesession['limitsoft']) {
								  				echo $onesession['limitsoft']-$sessioncount['total']; ?> Opening<?php if (($onesession['limitsoft']-$sessioncount['total']) > 1) echo 's'; ?> <div class="right"><?php echo anchor('registrations/new/'.$event['id'].'/'.$onesession['id'], 'Register &rarr;', 'class="btn btn-small blue"'); ?></div><?php
