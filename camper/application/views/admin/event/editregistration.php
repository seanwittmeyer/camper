...
/* 
 * Camper Admin Event / Registration / Edit View
 *
 * This is the ...
 *
 * File: /application/views/admin/event/editregistrations.php
...
?>
	...
	<article class="textsection">
		...
		<p>You can manage this unit registration here. As an admin, you can change registration numbers, set options/discounts, and enter payments. Any changes here do not need to be approved by an admin, nor are they limited by session/week limits. The leaders of this unit will be notified of the changes.</p>
		<?php if (!$message == '') { ?><div class="alert"><button type="button" class="close" data-dismiss="alert">&times;</button><?php echo $message; ?></div><?php } ?> 
		<div class="clear"></div>
		<?php if ($individual) { ?>
		<div class="camperform float " style="width:auto;" data-toggle="tooltip" title="Click to view <?php echo $unittitle; ?>"><a href="<?php echo base_url()."users/".$user->id; ?>"><span id="unit"><?php echo $unittitle; ?></span></a><label>Individual</label></div>
		<?php } else { ?>
		<div class="camperform float " style="width:auto;" data-toggle="tooltip" title="Click to view <?php echo $unittitle; ?>"><a href="<?php echo base_url()."units/".$unit['id']; ?>"><span id="unit"><?php echo $unittitle; ?></span></a><label>Unit</label></div>
		<?php } ?>
		...
		<div class="camperform float " style="width:auto;" data-toggle="tooltip" title="Click to edit this event's session"><a href="#modal_edit_session" data-toggle="modal"><span id="session"><?php echo (empty($session['title'])) ? $event['sessiontitle'].' '.$session['sessionnum'] : $session['title']; ?></span></a><label><?php echo $event['sessiontitle']; ?></label></div>
	...
	<?php echo form_open('payments/new');?>
	<article class="textsection">
		<div class="container">
			<div class="clear hr"></div>
			<div class="quarter">
				<h2>Payments &amp; Finances</h2>
				<p>View financial details, see payments and add payments for this registration.</p>
				<div class="clear"></div>
			</div>
			<div class="threequarter">
				<h2>Overview</h2>
   	    		<ul id="detailstabs" class="teal">
   					<li class="active"><a href="#finances" data-toggle="tab">Financial Details</a></li>
   					<li class=""><a href="#payments" data-toggle="tab">View Payments</a></li>
	...
	<article class="textsection">
		...
		<div class="container">
			<div class="quarter">
				<h2>Basics</h2>
				<p>Set the number of participants here. If a unit converts these numbers into a roster, you will not be able to edit the numbers. </p>
				...
				<p>You can remove this registration from the registration count and all classes by deactivating it.</p>
				<p>
					<?php echo ($reg['active'] == '0') ? anchor('api/v1/registration/activate?reg='.$reg['id'].'&return='.uri_string(), 'Activate?', 'class="btn teal" data-loading-text="Activating..." onclick="$(this).button(\'loading\');"'): anchor('api/v1/registration/deactivate?reg='.$reg['id'].'&return='.uri_string(), 'Deactivate?', 'class="btn red" data-loading-text="Deactivating..." onclick="$(this).button(\'loading\');"'); ?>
					<br>or <a data-toggle="popover" title="Are you sure?" data-placement="right" data-content="You are about to delete this registration. <br /><br />This means that the registration, it's roster, payments, and all class registrations will be removed as well.<br /><br /><a href='<?php echo base_url().'api/v1/registration/delete?reg='.$reg['id'].'&return=event/'.$event['id'].'/registrations'; ?>' class='btn red'>Delete</a>" class="camperpopover">delete this reg &rarr; </a>
				</p>
				<div class="clear"></div>
			</div>
			<div class="threequarter">
	...
	<div class="clear hr"></div>
		<div class="container">
			<div class="quarter">
				...
				<p>Options and discounts can be chosen here. When you make a change, the finances will be update and the unit leaders will be notified.</p>
				<input type="submit" name="submit" value="Save Changes" class="btn teal" data-loading-text="Saving..." onclick="$(this).button('loading');"  /> <input type="reset" name="reset" value="Reset" class="btn tan"  />
				<div class="clear"></div>
			</div>
			<div class="threequarter">
	...
	<div class="clear hr"></div>
		<div class="container">
			<div class="quarter">
				<h2>Roster &amp; Classes</h2>
				<p>These are the members added to the roster for this registration. Edit an individual's classes by clicking edit.</p>
				<p><input type="submit" name="submit" value="Save Roster" class="btn teal" data-loading-text="Saving..." onclick="$(this).button('loading');"  /> <input type="reset" name="reset" value="Reset" class="btn tan"  /></p>
				<p>You can print or save the adult and youth rosters for this registration.</p>
				<p><?php echo anchor('api/v1/unitroster/'.$reg['id'].'.pdf', '<i class="icon-file"></i> Unit Roster &rarr;', 'class="btn tan"'); ?></p>
				
				<div class="clear"></div>
			</div>
			<div class="threequarter">
				<ul id="rosterstabs" class="detailstabs teal">
   					<li class="active"><a href="#roster-youth" data-toggle="tab">Youth</a></li>
   					<li class=""><a href="#roster-adults" data-toggle="tab">Adults</a></li>
   					<li class=""><a href="#roster-array" data-toggle="tab">array</a></li>
   				</ul>
   	    		<div class="tab-content">
   					...
   					<div class="tab-pane fade" id="roster-array">
   						<pre><?php print_r($members); ?></pre>
   					</div>
   	    		</div>
   				<div class="clear"></div>
			</div>
	...
	<?php echo form_close();?> 
	<?php endif; /* end if isset unit id */ ?>
   	<!-- Edit Time Modal -->
   	<article class="content">
		<div id="modal_edit_time" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<?php echo form_open(uri_string());?>
			...
			<input type="hidden" name="session" value="<?php echo $session['id']; ?>" />
	   		<div class="container">
	       		...
	       		<div class="pull">
	   	    		<h2 class="pull">Time Editor</h2>
	   	    		<p>Change when the registration was added to the system.</p>
	   	    		<div class="clear"></div>
	       		</div>
	       		<div class="tab-content inner-push">
	   				<h2 class="section">Change the Time of Registration</h2>
	   				<p>The time of registration helps us know how to bill the unit for the event. Some events that include early bird discounts use this to qualify for the discounts as well. Set a new registration time (the time will be 12am on the date you choose). Leave the box blank to leave it as it is.</p>
	   				<p><?php echo $unit['unittype'].' '.$unit['number']; ?>'s current time of registration is <strong><?php echo date('F j, Y g:ia', $reg['registerdate']['time']); ?></strong></p>
	   		   		<div class="camperform float search" style="width: 60%" ><i class="icon-calendar"></i><input class="ico datepicker" type="text" name="time" id="ftime" placeholder="Click to set a new time..." /><label for="ftime">Time of Registration</label></div>
		   			<div class="clear"></div>
		   			<p><input type='submit' name='submit' value='Update Time' class='btn teal' /> <button class="btn tan" data-dismiss="modal" aria-hidden="true">Nevermind</button></p>
	   			</div>
	   		</div>
	   		<div class="clear"></div>
	   		<?php echo form_close();?>
   	...
   	<!-- Edit Session Modal -->
   	<article class="content">
		<div id="modal_edit_session" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<?php echo form_open(uri_string());?>
			<?php if ($individual) { ?>
			<input type="hidden" name="user" value="<?php echo $user->id; ?>" />
			<input type="hidden" name="individual" value="1" />
			...
			<input type="hidden" name="reg" value="<?php echo $reg['id']; ?>" />
	   		<div class="container">
	   			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
	       		<div class="pull">
	   	    		<h2 class="pull">Session Editor</h2>
	   	    		<p>Change this registration's session.</p>
	   	    		<div class="clear"></div>
	       		</div>
	       		<div class="tab-content inner-push">
	   				<h2 class="section">Change the <?php echo $event['sessiontitle']; ?></h2>
	   				<p>You can change the session for this registration here. When you select a new session the site will also update all payments, rosters, and class registrations, if any. The leader will be notified when this changed as well.</p>
	   				<p><?php echo $unit['unittype'].' '.$unit['number']; ?>'s current <?php echo lcfirst($event['sessiontitle']); ?> is <strong><?php echo (empty($session['title'])) ? $event['sessiontitle'].' '.$session['sessionnum'] : $session['title']; ?></strong></p>
	   		   		<div class="camperform float" style="width: 60%" >
	   					<select id="fsession" name="session" data-toggle="tooltip" title="Choose the new session">
						<?php foreach ($sessions as $onesession): ?>
							<option<?php if ($session['id'] == $onesession['id']) echo ' selected="selected"'; ?> value="<?php echo $onesession['id']; ?>"><?php echo (empty($onesession['title'])) ? $event['sessiontitle'].' '.$onesession['sessionnum'] : $onesession['title']; ?></option>
						<?php endforeach; ?>
	   					</select>
						<label for="fsession">Session</label></div>
		   			<div class="clear"></div>
		   			<p><input type='submit' name='submit' value='Change Session' class='btn teal' /> <button class="btn tan" data-dismiss="modal" aria-hidden="true">Nevermind</button></p>
	   			</div>
	   		</div>
	   		<div class="clear"></div>
	   		<?php echo form_close();?>
