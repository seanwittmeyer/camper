...
?>
	<div class="subnav">
		<div class="container">
			...
			<nav class="campersubnav">
				<li class="active"><?php echo anchor("public/events", 'Upcoming Events');?></li>
			</nav>
		</div>
	</div>
	<article class="textsection">
		<div class="container">
			...
			<p>These are all of the upcoming events open for registration in Camper. This includes summer camps, council and district events like weekends and camporees, and even trainings. Event registration is easy, simply <?php echo anchor('start', 'create an account'); ?> or <?php echo anchor('signin', 'sign in'); ?> to get started.</p>
			<p><strong>Already registered?</strong> If you have registered for an event already, simply <?php echo anchor('signin', 'sign in'); ?> to manage your registration.</p>
		</div>
   		<div class="container">
					...
					<div id="upcoming<?php echo $i; ?>" class="accordion-body collapse ">
						<div class="accordion-inner">
							...
							<div class="desc left">
								...
								<?php else : ?><p><?php echo ($sessions[0]['open'] == '1' && $sessions[0]['count'] < $sessions[0]['limitsoft']) ? anchor("signin", 'Sign in &rarr;', 'class="btn blue"') : '<div class="alert">Registration is closed since this event is full</div>'; ?></p><?php endif; ?>
							</div>
											...
											<td><?php foreach ($groups['groups'] as $g) { 
												...
												if ($onesession['open'] == '1') {
													...
													if ($g['__limit'] == 0 || $g['__limit'] == false) {
														...
														$g['__openings'] = '';
														$g['__message'] = '';
														$g['__button'] = anchor('signin', 'Sign in &rarr;', 'class="btn btn-small blue camperhoverpopover" data-toggle="popover" data-placement="top" data-content="There is no registration limit for this '.strtolower($event['sessiontitle']).'<br /><br />Click to register, you can set your numbers on the next page."');
														...
														$g['__message'] = '';
														$g['__button'] = anchor('signin', 'Sign in &rarr;', 'class="btn btn-small blue camperhoverpopover" data-toggle="popover" data-placement="top" data-content="<strong>'.$g['__openspots'].'</strong> Openings!"');
													} else {
														// Group Full
										...
										<?php $j=1; foreach ($sessions as $onesession): $sessioncount = $this->shared->count_session($onesession['id']); ?> 
							      	    <tr>
								  			...
								  			<td><?php if ($onesession['open'] == '1' && $sessioncount['total'] < $onesession['limitsoft']) {
								  				echo $onesession['limitsoft']-$sessioncount['total']; ?> Opening<?php if (($onesession['limitsoft']-$sessioncount['total']) > 1) echo 's'; ?> <div class="right"><?php echo anchor('signin', 'Sign in &rarr;', 'class="btn btn-small blue"'); ?></div><?php
