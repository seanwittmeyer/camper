...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array(), $emailtemplate='default') //need to test email activation
	{
		...
		else
		{
			...
			{
				$data['user'] = $user;
				if ($emailtemplate == 'admincreated') {
					$message = $this->load->view($this->config->item('email_templates', 'ion_auth').'admincreated.tpl', $data, true);
				} else {
