...
class Auth extends CI_Controller {
	...
	function login()
	{
		$go = ($this->input->get('go')) ? $this->input->get('go'): 'dashboard';
		if ($this->ion_auth->logged_in())
		{
			//redirect them to the login page
			redirect($go, 'refresh');
		...
		if ($this->form_validation->run() == true)
		{
			...
			//forward to $go uri if set, or the dashboard/home page
			if ($this->input->post('go')) {
				$go = $this->input->post('go');
			} else {
				$go = (!$this->ion_auth->is_admin()) ? 'home': 'dashboard';
			}
			...
			if ($this->ion_auth->login(strtolower($this->input->post('identity')), $this->input->post('password'), $remember))
			{
				...
				//redirect('/', 'refresh');
				if ($this->shared->new_to_camper($this->input->post('identity'))) redirect($go, 'refresh');
				if (!$this->ion_auth->is_admin()) redirect($go, 'refresh');
				redirect($go, 'refresh');
			}
			else
			{
				...
				redirect('signin?go='.$go, 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
			...
			//set the flash data error message if there is one
			$data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$data['successmessage'] = $this->session->flashdata('success');
			$data['pw'] = $this->session->flashdata('pw');
			$data['em'] = $this->session->flashdata('identity');
			...
			if ($this->input->post('go')) {
				...
				$data['go'] = (!$this->ion_auth->is_admin()) ? 'home': 'dashboard';
			}
			$data['page'] = 'Sign In';
			$data['section'] = 'signin';
			$data['breadcrumbs'] = anchor('signin','Sign In');
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$data['section'] = 'none';
			if (!$this->ion_auth->logged_in()) { $this->load->view('templates/header_public', $data); } elseif (!$this->ion_auth->is_admin()) { $this->load->view('templates/header_leader', $data); } else { $this->load->view('templates/header_admin', $data); }
	...
	function activate($id, $code=false)
	{
		...
		if ($activation)
		{
			...
			if ($this->ion_auth->is_admin()) {
				$this->session->set_flashdata('message', 'The account has been activated.');
				redirect("users/$id", 'refresh');
			} else {
				$this->session->set_flashdata('message', 'Your account is now active, sign in to use Camper.');
				redirect("signin", 'refresh');
			}
		}
		else
	...
	function deactivate($id = NULL)
	{
		...
		else
		{
			...
			$this->session->set_flashdata('message', 'Deactivation was successful.');
			redirect("users/$id", 'refresh');
		}
	}
